...
- **Advanced** — the per-platform knobs that already have working defaults (reply mode, allow-all, home channel + its display name, proxy) collapse behind an **Advanced** disclosure so the form is just the fields you actually have to fill in. Expand it only when you want to override a default. Fields with a fixed set of values render as a picker showing the effective default, and free-text fields show that default as their placeholder — so a blank box tells you what Hermes will do rather than just naming the env var. The home channel in particular does not need to be set here: the first time you talk to the bot it offers to make that chat the home channel via `/sethome`.
